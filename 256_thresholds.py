...
class Test_Experiment:
    ...
    def run(self, mode = "random", strategy="eliminate_negatives",max_tests = 100000,target_ratio=2):
            ...
            for t in range(1, max_tests + 1):
                ...
                actual_test_size = len(test)
                test_inaccuracy_ratio = actual_test_size / self.test_size
                ...
                self.history.append({
                    ...
                    "coded_test_size": self.test_size,
                    "test_inaccuracy_ratio": test_inaccuracy_ratio
                })

                # New Stop Condition: stop when ratio...
                if fp_ratio <= target_ratio:
                    break
...
def tests_to_reach_threshold(history, threshold, max_tests=80000):
    for h in history:
        if h["fp_ratio"] <= threshold:
            return h["tests so far"], True
    return max_tests, False
...
def run_full_experiment_suite(positions, infected_set, test_size):
    ...
    for algo_name, mode in algorithm_modes.items():
        ...
        history_df = pd.DataFrame(exp.history)

        competitive_test_inaccuracy_ratio = history_df["test_inaccuracy_ratio"].mean()
        average_actual_test_size = history_df["actual_test_size"].mean()
        ...
        results_summary.append({
            ...
            "Average_Actual_Test_Size": average_actual_test_size,
            "Competitive_Test_Inaccuracy_Ratio": competitive_test_inaccuracy_ratio
        })
...
def run_fp_threshold_experiment(
    positions,
    infected_set,
    test_size,
    fp_thresholds,
    ...
    for algo_name, mode in algorithm_modes.items():
        ...
        for threshold in fp_thresholds:
            ...
            for rep in range(repetitions):
                ...
                exp = Test_Experiment(
                    ...
                    infected_set=infected_set,
                    test_size=test_size,
                    label=f"{algo_name}_t{test_size}_r{threshold}",
                    seed=rep
                )
                ...
                tests_list.append(len(exp.tests))

            results.append({
                "Algorithm": algo_name,
                "Test_Size": test_size,
                "FP_Threshold": threshold,
                "Average_Total_Tests": np.mean(tests_list),
                "Min_Tests": np.min(tests_list),
                "Max_Tests": np.max(tests_list)
            })
...
def plot_tests_vs_fp_threshold(df):
    ...
    for algo in df["Algorithm"].unique():
        ...
        plt.plot(
            subset["FP_Threshold"],
            subset["Average_Total_Tests"],
            marker="o",
            label=algo
        )

    plt.xlabel("FP Ratio Threshold")
    plt.ylabel("Average Number of Tests")
    plt.title("Tests Required vs FP Ratio Threshold")
    plt.legend()
    plt.grid(True)
    plt.show()
...
def save_summary_append(summary, filename="grid256_size_32.xlsx"):
...
def run_threshold_grid_experiment(
    positions,
    infected_set,
    test_sizes,
    fp_thresholds,
    ...
    for algo_name, mode in algorithm_modes.items():
        ...
        for test_size in test_sizes:
            ...
            for rep in range(repetitions):
                ...
                print(f"Running {algo_name}, test_size={test_size}, rep={rep}")

                exp = Test_Experiment(
                    positions=positions,
                    infected_set=infected_set,
                    test_size=test_size,
                    label=f"{algo_name}_t{test_size}_rep{rep}",
                    seed=rep
                )
                ...
                competitive_test_inaccuracy_ratio = history_df["test_inaccuracy_ratio"].mean()
                avg_actual_test_size = history_df["actual_test_size"].mean()

                raw_runs.append({
                    "Algorithm": algo_name,
                    "Test_Size": test_size,
                    "Repetition": rep,
                    "Total_Tests_Run": len(exp.tests),
                    "Final_FP_Ratio": exp.history[-1]["fp_ratio"],
                    "Final_FP": exp.history[-1]["false positives"],
                    "Final_FN": exp.history[-1]["false negatives"],
                    "Average_Actual_Test_Size": avg_actual_test_size,
                    "Competitive_Test_Inaccuracy_Ratio": competitive_test_inaccuracy_ratio
                })

                for threshold in fp_thresholds:

                    tests_needed, reached = tests_to_reach_threshold(
                        exp.history,
                        threshold,
                        max_tests=max_tests
                    )
    ...
    inaccuracy_df = pd.DataFrame(inaccuracy_results)

    summary_df = threshold_df.groupby(
        ["Algorithm", "Test_Size", "FP_Threshold"],
        as_index=False
    ).agg(
        Average_Tests=("Tests_Needed", "mean"),
        Min_Tests=("Tests_Needed", "min"),
        Max_Tests=("Tests_Needed", "max"),
        Success_Rate=("Reached", "mean")
    )

    best_df = summary_df.loc[
        summary_df.groupby(["Algorithm", "FP_Threshold"])["Average_Tests"].idxmin()
    ].reset_index(drop=True)

    best_inaccuracy_df = best_df.merge(
        inaccuracy_df.groupby(
            ["Algorithm", "Test_Size", "FP_Threshold"],
            as_index=False
        ).agg(
            Average_Actual_Test_Size=("Average_Actual_Test_Size", "mean"),
            Competitive_Test_Inaccuracy_Ratio=("Competitive_Test_Inaccuracy_Ratio", "mean")
        ),
        on=["Algorithm", "Test_Size", "FP_Threshold"],
        how="left"
    )

    return raw_df, summary_df, best_df, best_inaccuracy_df

def save_threshold_results_to_excel(
    raw_df,
    summary_df,
    best_df,
    best_inaccuracy_df,
    filename="grid256_threshold_analysis.xlsx"
):

    with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:

        raw_df.to_excel(writer, sheet_name="Raw_Runs", index=False)
        summary_df.to_excel(writer, sheet_name="All_TestSizes", index=False)
        best_df.to_excel(writer, sheet_name="Best_TestSize", index=False)
        best_inaccuracy_df.to_excel(writer, sheet_name="Best_Inaccuracy", index=False)

    print(f"✅ Results saved to {filename}")

def plot_best_tests_vs_fp_threshold(best_df):

    plt.figure(figsize=(10, 6))

    for algo in best_df["Algorithm"].unique():

        subset = best_df[best_df["Algorithm"] == algo]

        plt.plot(
            subset["FP_Threshold"],
            subset["Average_Tests"],
            marker="o",
            label=algo
        )

    plt.xlabel("FP Ratio Threshold")
    plt.ylabel("Minimum Average Tests")
    plt.title("Best Test Size Performance by FP Threshold")
    plt.legend()
    plt.grid(True)
    plt.show()

def plot_all_test_sizes(summary_df):
    ...
    for algo in summary_df["Algorithm"].unique():
        ...
        for test_size in sorted(subset["Test_Size"].unique()):

            ts_data = subset[subset["Test_Size"] == test_size]

            plt.plot(
                ts_data["FP_Threshold"],
                ts_data["Average_Tests"],
                marker="o",
                label=f"t={test_size}"
            )

        plt.xlabel("FP Ratio Threshold")
        plt.ylabel("Average Tests")
        plt.title(f"Average Tests vs FP Threshold - {algo}")
        plt.legend(fontsize=7)
        plt.grid(True)
        plt.show()
